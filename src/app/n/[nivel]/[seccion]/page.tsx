...
import { curso, gramaticas, nivelCurso, seccionCurso, unidad } from "@/lib/contenido";
      ...
      <main className="envoltorio">
        <section style={{ padding: "18px 0 14px" }}>
          ...
          <p className="tenue" style={{ margin: "4px 0 0" }}>
            {s.palabras} palabras en {s.unidades.length} unidades
            {gram.length ? ` · ${gram.length} puntos de gramática` : ""}
          </p>
        </section>
        <ListaUnidades nivel={nivel} unidades={s.unidades} gramatica={gram} />
      </main>
    </>
  );
}
